25.times do
  ...
  fulfillment_fee: Faker::Commerce.price(range: 5..12.99),
  unit_cost: Faker::Commerce.price(range: 5..12.99)
end

5000.times do 
  order_type = %w[confirmed shipped delevered].sample
  purchase_date = Faker::Time.backward(days: 30)
  shipped_at = %w[shipped delevered].include?(order_type) ? purchase_date + 3.days : nil
  ...
  payment_method: %w[credit_card paypal].sample,
